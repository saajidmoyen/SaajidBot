...
public class TBot extends PircBot {
	...
	// VARIABLES-----------------------------------------//
	// booleans
	private boolean isPlaying; // keeps track of whether the channel is in
								// "playing mode"
	private boolean isJoining; // keeps track of whether the players are
								// currently joining the game
	private boolean questionRight; // keeps track of thether the last question
									// was correctly answered
	private boolean negs; // determines if negs are in play
	private boolean finishedReading; // notes if the bot has "finished reading"
										// it's question.
	private boolean showCategory; // tells bot whether to show the category
									// beforehand or not
	private boolean askBySent; // determines if the bot posts in sentences or by
								// X number of words
	private boolean hasNegged; // sees if someone has already negged this
								// question
	private boolean paused; // sees if the game is paused
	private boolean answeringBonus; // Whether teams are answering a bonus or
									// not
	...
	private boolean startBonus; // Whether a bonus should be started (i.e. a
								// player got the question right)
	private boolean readFirstBonusPart; // Whether we should read the first
										// bonus part or not... false if buzz on
										// bonus leadin
	private boolean forceBonus; // Force a bonus to be read
	private boolean inOrder; // Whether we're reading the set in order
	private boolean forceMode; // Requires a '!next' command before moving to
								// the next question
	private boolean bonusPaused; // Whether to start the next question or not
	private boolean attemptReconnect; // Whether the bot should try reconnecting
										// after a disconnect

	private int roundNum, questionTime, seconds, rounds; // seconds = time since
															// end of question
	...
	private HashSet<Player> allPlayers;// our set of all of the players who have
										// played - for stats
	private HashSet<String> comment;// our set of comments
	private HashSet<Pattern> prompts; // holds a list of answers the player has
										// said that has been prompted
	...
	private String setsFile; // represents the file that contains all of the
								// playable sets
	private String bonusSetsFile; // Represents the file that contains all of
									// the playable bonus sets
	...
	private Pattern allowedQ; // Determines which question categories are
								// allowed
	private Pattern unallowedQ; // Determines which question categories are not
								// allowed.
	...
	// GameThreads
	private static GameThread maingame; // Made static since there should only
										// be one instance of it.

	// Joins
	private Join joining; // Used as a timer for the joining period. --> make
							// static in future?

	// Countdown timer
	private Countdown cdTimer; // used as a timer to limit the amount of time to
								// accept an answer after a buzz
	...
	// Randoms
	private static Random gen = new Random();// a random object for all-purpose
												// use
	...
	private void init() {
		...
		// These boolean values only matter during each question or the
	...
	private void loadSettings() {
		try {
			...
			while (true) {
				...
				if (line == null || line.equals("~"))
					...
					case 12:
						...
						break;
					case 13:
						allowedQ = Pattern.compile(line,
								Pattern.CASE_INSENSITIVE);
						break;
					case 14:
						unallowedQ = Pattern.compile(line,
								Pattern.CASE_INSENSITIVE);
						break;
					case 15:
						showCategory = bint;
					...
					default:
						System.out
								.println("Problem with your settings.txt file.  "
			...
			reader.close();
			if (numWordsPerLine == 0) // if this is true, then numWordsPerLine
										// was never initialized, so we have a
										// problem.
			{
				// TODO: Make into a private method to remove code duplication?
				/* msg */System.out
						.println("Incomplete settings.txt file.  Shutting down...");
	...
	// --------------------msg(String text): emits a message in the current
	// channel-------//
	private void msg(String text) {
		// sends a message on a white background with dark blue text
		// the package name is in here because I kept getting a weird
		// "wrong class" error
		sendMessage(botChannel, WHITE + DARK_BLUE + text);
	}

	// --------------------write(BufferedWriter writer, String text): writes to
	// the file w/ a new line-------//
	...
	// --------------------loadSet(String): loads the questionset from
	// setName------------//

	// loadSet --> Read qSets, add ".*" to the user's preference, and find any
	// set that applies to it.
	...
	private void loadSet(String setName) {
		...
		// We add ".*" to setName so that it accepts everything containing
		// setName + misc text
		...
		for (int i = 0; i < sets.length; i++) // Go through the sets and see if
												// they match the Pattern
		{
			if (setToLoad.matcher(sets[i]).matches()) {
				counter = 0;
				try {
					reader = new BufferedReader(new FileReader(setsDirectory
							+ sets[i].trim() + ".txt"));
					String line;
					while (true) {
						line = reader.readLine();
						if (line == null)
							break;
						// Create a temporary question, and if it matches
						// accepted questions (and not unallowed questions),
						// then ID it and add it to the questions list.
						Question temp = new Question(line);
						if (allowedQ.matcher(temp.getTopic()).matches()
								&& !unallowedQ.matcher(temp.getTopic())
										.matches()) {
					...
					msg(sets[i].trim() + ".txt does not exist.");
				}
			} // end of if
		} // end of for

		// check to see if the set is empty. If it is, reset the regular
		// expression to .*, and reload the set.
		if (questions.size() == 0 && !allowedQ.pattern().equals(".*")) {
			msg(allowedQ.pattern()
					+ " is not contained in this set.  Reloading set with all topics allowed.");
	...
	private void loadBonuses(String setName) {
		...
		for (int i = 0; i < bonusSets.length; i++) // Go through the sets and
	...
	private void initSets() {
		// setsFile should just be a one line file with a list of sets split up
		// by commas.
		try {
			BufferedReader reader = new BufferedReader(new FileReader(
					setsDirectory + setsFile));
			...
			// If we can't find setsFile, there's no point in having the bot, so
			// exit.
	...
	// ------------onMessage(...): Calls on helper methods for various
	// tasks--------------//
	// synchronized makes it slower, but it allows us to pause the game
	protected synchronized void onMessage(String channel, String sender,
			String login, String host, String text) {
		text = text.trim();

		// if "buzz" is entered during the round, prompts the user for an answer
		if (isPlaying && answerTime == 0 && !questionRight && !paused
				&& text.trim().toLowerCase().equals("buzz")
				&& getPlayer(sender) != null
		...
		// checks for the validity of an entered answer and assigns points on
		// that basis
		// All of the comments for commands can be found on the documentation
		// page.
		else if (isPlaying && sender.equals(answerer)
				&& !text.equalsIgnoreCase("buzz") && getPlayer(sender) != null
				&& !getTeam(getPlayer(sender).getTeam()).hasGuessed()
				&& questionRight == false) // every possible thing to do with an
											// answer
		{
			Player tempPlayer = getPlayer(sender);
			...
			{
				stopQuestion();
				msg(sender + " gets it with " + text + ".  Possible Answers: "
						+ tempQuestion.getAnswersString() + " (ID: " + tempID
						+ " " + tempSet + ")");
				...
				if (!answeringBonus) {
					...
					points = 10;
					if (useBonuses) { // Set which team will use this bonus
						bonusTeam = getTeam(tempPlayer.getTeam());
						startBonus = true; // Successfully answered, so start
											// the bonus
					}
				} else {
					points = curbonus.getPoints(curBonusPart);
					bonusTeam.addBonusPoints(points);
					readFirstBonusPart = false; // Don't read the first bonus
												// part if we already did
				}
			...
			// see if it's promptable
				...
				answerer = sender;
					...
					&& text.length() > 0 && !text.substring(0, 1).equals("!")) {
				answerer = "";
				answerTime = 0;
				msg(sender + " [" + tempPlayer.getTeam() + "] guessed " + text
						+ ", which is incorrect.");
		...
		// figuring out what was just posted
			...
			if (asker != null) {
				// Op-only commands: !clearcom, !exit, !loadplayer, !set
				// messageDelay
				if (asker.isOp())
	...
	private void regularCommands(String channel, String sender, String login,
			String host, String text) {
		if (text.length() > 5 && text.substring(1, 6).equalsIgnoreCase("start")
				&& !isPlaying && !isJoining)
			startJoining(text);

		// if "!join" is uttered by a channel member, followed by a team name,
		// he is added to the team
		else if (text.length() > 5
				&& text.substring(1, 5).equalsIgnoreCase("join")
				&& (isJoining || isPlaying))
			addToTeam(sender, text);

		else if (text.equalsIgnoreCase("!random") && (isJoining || isPlaying))
			addToTeam(sender,
					"!join " + teams[gen.nextInt(teams.length)].getName());
		...
		// TODO: Move to !get ?
			...
			msg("Number of tossups: " + Integer.toString(questions.size())
					+ ", " + "number of bonuses: "
					+ Integer.toString(bonuses.size()));

		// more specific: gets the size of a certain category
		else if (text.length() > 5
				&& text.substring(1, 5).equalsIgnoreCase("size")) {
			String category = text.substring(5, text.length()).trim()
					.toLowerCase();
			int cases = 0;
			...
			for (int i = 0; i < questions.size(); i++)
				if (category.equals(questions.get(i).getTopic().toLowerCase()))
					cases++;
			msg("Number of " + category + " tossups: "
					+ Integer.toString(cases) + ", number of " + category
					+ " bonuses: " + Integer.toString(bonuses.size()));
		}

		// PAUSE FUNCTION: see if this needs to be regulated. For now, make it
		// !pause and !unpause
		...
		else if (text.equalsIgnoreCase("!score") && isPlaying) // deprecated,
																// but still
																// here
			msg("Team " + teams[0].getName() + ": "
					+ Integer.toString(teams[0].getScore()) + ", Team "
					+ teams[1].getName() + ": "
					+ Integer.toString(teams[1].getScore()));

		else if (text.length() > 8
				&& text.substring(1, 8).equalsIgnoreCase("comment")) {
			comment.add(sender + ": " + text.substring(8, text.length()).trim());
			msg("Comment added.");
		}

		else if (text.length() > 5
				&& text.substring(1, 5).equalsIgnoreCase("team")
				&& getPlayer(text.substring(6, text.length()).trim()) != null) {
			...
			msg(teams[0].getName() + teams[0].getPlayers().toString() + " and "
					+ teams[1].getName() + teams[1].getPlayers().toString());

		// get stats on a user
		else if (text.length() > 6
				&& text.substring(1, 6).equalsIgnoreCase("stats")) {
			if (getPlayerFromAllPlayers(text.substring(6, text.length()).trim()) != null)
				msg(getPlayerFromAllPlayers(
						text.substring(6, text.length()).trim()).getStats());
			else
				msg(text.substring(6, text.length()).trim()
						+ " does not have a stats file loaded.");
		}

		else if (text.length() > 9
				&& text.substring(1, 9).equalsIgnoreCase("curstats")) {
			if (getPlayer(text.substring(9, text.length()).trim()) != null)
				msg(getPlayer(text.substring(9, text.length()).trim())
						.getCurStats());
			else
				msg(text.substring(9, text.length()).trim()
						+ " is not playing in this game.");
		}

		// Exception: Players can remove themselves, gamemasters can remove
		// players, and ops can remove players
		else if (text.length() > 7
				&& text.substring(1, 7).equalsIgnoreCase("remove")
			...
			if (sender.equals(rem.getName())
					|| getPlayer(sender).equals(gamemaster) || isOp(sender)) {
				if (rem != null) {
					msg("Player " + rem.getName() + " removed.");
					// stops if no one is playing anymore--player is removed in
					// stillPlayers
					stillPlayers(rem.getName());

					// If the player was not removed in stillPlayers, remove
					// them now
					// Note that rem is still a valid player if the condition
					// holds
		...
		// retrieves certain variables. Some variables can only be accessed by
		// an op.
		// variables that use get: negs, rounds, delay, minMVP, gameswoReload,
		// limwoReload
		// comment, players, allPlayers, allowedQ, unallowedQ
		// last part of if-statement there to allow for getq, which requires a
		// parameter.
		else if (text.length() > 5
				&& text.substring(1, 4).equalsIgnoreCase("get")
				&& !text.substring(1, 5).equalsIgnoreCase("getq")) {
			...
			if (get.equalsIgnoreCase("negs"))
				...
				msg(teams[0].getName() + teams[0].getPlayers().toString()
						+ " and " + teams[1].getName()
						+ teams[1].getPlayers().toString());
				...
				msg("Character message delay: " + Integer.toString(delay)
						+ " milliseconds.");
			else if (get.equalsIgnoreCase("minMVP")
					|| get.equalsIgnoreCase("mvp"))
				msg("Minimum number of players needed for MVP/LVP stats: "
						+ Integer.toString(minMVP) + ".");
			else if (get.equalsIgnoreCase("gamesworeload"))
				msg("Current number of games without a reload: "
						+ Integer.toString(gameswoReload) + ".");
			else if (get.equalsIgnoreCase("limworeload"))
				msg("Maximum number of games without a reload: "
						+ Integer.toString(limwoReload) + ".");
			...
			else if (get.equalsIgnoreCase("comments")
					|| get.equalsIgnoreCase("com"))
				// This now iterates through the comments and sends them one by
				// one to the sender
			...
			else if (get.equalsIgnoreCase("showcategory")
					|| get.equalsIgnoreCase("category"))
				msg("Show Category is set to " + Boolean.toString(showCategory));
			else if (get.equalsIgnoreCase("answerdelay"))
				msg("Time for answering questions set to: "
						+ Integer.toString(answerDelay) + " seconds.");
			else if (get.equalsIgnoreCase("breakBtwQ"))
				msg("Time delay between questions: "
						+ Integer.toString(breakBtwQ) + " milliseconds.");
			else if (get.equalsIgnoreCase("messageDelay"))
				msg("Message delay set to " + Long.toString(getMessageDelay())
						+ " milliseconds.");
			else if (get.equalsIgnoreCase("numWordsPerLine"))
				msg("Number of words to send per line: "
						+ Integer.toString(numWordsPerLine) + ".");
				...
				msg("Maximum number of players per team: "
						+ Integer.toString(playersPerTeam));
				...
				msg("TriviaBot "
						+ VERSION
						+ " © 2006-2009 Alejandro Lopez-Lago & Raghav Puranmalka");
			...
			else if (get.length() > 8
					&& get.substring(0, 8).trim().equalsIgnoreCase("question")
					&& isOp(sender)) {
				try {
					msg((questions.get(Integer.valueOf(get.substring(8,
							get.length()).trim()))).toString());
					...
					&& get.substring(0, 2).equalsIgnoreCase("q ")
					&& isOp(sender)) {
				try {
					msg((questions.get(Integer.valueOf(get.substring(2,
							get.length()).trim()))).toString());
				} catch (IndexOutOfBoundsException e) {
					msg("Invalid syntax");
				} catch (NumberFormatException e) {
			...
			// gets the question and answer frequency for a particular term -
			// put into a different method next version?
			else if (get.length() > 1
					&& get.substring(0, 4).equalsIgnoreCase("freq")) {
				String search = get.substring(4, get.length()).trim()
						.toLowerCase();
				int Qfreq = 0;
				int Afreq = 0;
				for (Question q : questions) {
					if (q.getQuestion().indexOf(search) >= 0) {
						...
						while (counter < quest.length()
								&& quest.indexOf(search + " ", counter) >= 0) {
				...
				msg("Question frequency: "
						+ Integer.toString(Qfreq)
						+ " | Answer Frequency: "
						+ Integer.toString(Afreq)
						+ " | Weighted Frequency: "
						+ Double.toString((100 * (Qfreq + 5.0 * Afreq) / questions
								.size())));
				...
				msg("Break between bonus parts: " + breakBtwBonuses
						+ " milliseconds.");
			else
				// tell the user their command does exist
				msg("You can't use !get on that.");
		}
	}

	private void gamemasterCommands(String channel, String sender,
			String login, String host, String text) {
		...
		// Resets bot variables to their default state
			...
			loadSettings();
		// variables that use get: negs, delay, minMVP, gamesworeReload,
		// limwoReload, allowedQ, unallowedQ, showCategory,
		// answerDelay, breakBtwQ, [messageDelay for ops]
		else if (text.length() > 5
				&& text.substring(1, 4).equalsIgnoreCase("set")) {
			String[] arguments = text.split(" "); // this should create an array
													// with size 3
			...
			for (int i = 3; i < arguments.length; i++)
				param += " " + arguments[i];
			// the first part sees what variable needs to be modified, and the
			// second part
			...
			try {
				...
				if (set.equalsIgnoreCase("negs"))
				...
				else if (set.equalsIgnoreCase("delay")
						&& Integer.valueOf(param).intValue() > 0
						&& Integer.valueOf(param).intValue() < Integer.MAX_VALUE) {
					delay = Integer.valueOf(param).intValue();
					msg("Character message delay set to: "
							+ Integer.toString(delay) + " milliseconds.");
				} else if ((set.equalsIgnoreCase("minMVP") || set
						.equalsIgnoreCase("mvp"))
						&& Integer.valueOf(param).intValue() > 1) {
					minMVP = Integer.valueOf(param).intValue();
					msg("Minimum number of players needed for MVP/LVP stats set to: "
							+ Integer.toString(minMVP) + ".");
				} else if (set.equalsIgnoreCase("gamesworeload")
						&& Integer.valueOf(param).intValue() > 0) {
					gameswoReload = Integer.valueOf(param).intValue();
					msg("Current number of games without a reload set to: "
							+ Integer.toString(gameswoReload));
				} else if (set.equalsIgnoreCase("limworeload")
						&& Integer.valueOf(param).intValue() > 0) {
					limwoReload = Integer.valueOf(param).intValue();
					msg("Maximum number of games without a reload set to: "
							+ Integer.toString(limwoReload));
				} else if (set.equalsIgnoreCase("allowedQ")) {
					// TODO: Figure out exception and catch it explicitly
					try {
						allowedQ = Pattern.compile(param,
								Pattern.CASE_INSENSITIVE);
						loadSet(qSetFile);
						loadBonuses(bSetFile);
						msg("Allowed questions: " + allowedQ.toString());
					...
					try {
						unallowedQ = Pattern.compile(param,
								Pattern.CASE_INSENSITIVE);
						loadSet(qSetFile);
						loadBonuses(bSetFile);
						msg("Unallowed questions: " + unallowedQ.toString());
				...
				} else if (set.equalsIgnoreCase("showCategory")
						|| set.equalsIgnoreCase("category")) {
					showCategory = Boolean.valueOf(param);
					msg("ShowCategory is set to: "
							+ Boolean.toString(showCategory));
				} else if (set.equalsIgnoreCase("answerDelay")
						&& Integer.valueOf(param).intValue() > 0
						&& Integer.valueOf(param).intValue() < Integer.MAX_VALUE) {
				...
				} else if (set.equalsIgnoreCase("numWordsPerLine")
						&& Integer.valueOf(param).intValue() > 0
						&& Integer.valueOf(param).intValue() < Integer.MAX_VALUE) {
					numWordsPerLine = Integer.valueOf(param).intValue();
					msg("Number of words sent per line is now: "
							+ Integer.toString(numWordsPerLine) + ".");
				} else if (set.equalsIgnoreCase("rounds")
						&& Integer.valueOf(param).intValue() >= roundNum
						&& Integer.valueOf(param).intValue() > 0)// &&
																	...
																	// Integer.MAX_VALUE)
				{
					...
					if (rounds < questions.size() && rounds > 0
							&& (!useBonuses || rounds < bonuses.size())) {
						// change the halftime variable - MAYBE BREAKABLE
						// LOCATION?
						halftime = rounds / 2;
						msg("Number of rounds in the game: "
								+ Integer.toString(rounds) + ".");
					} else {
						rounds = oldRounds;
						msg("Improper input: number of rounds must be less than the size of the question database.");
					}
				} else if (set.equalsIgnoreCase("playersPerTeam")
						&& Integer.valueOf(param).intValue() > 0
						&& Integer.valueOf(param).intValue() < Integer.MAX_VALUE) {
					playersPerTeam = Integer.valueOf(param).intValue();
					msg("Maximum number of players per team: "
							+ Integer.toString(playersPerTeam) + ".");
				} else if (set.equalsIgnoreCase("timeToJoin")
						&& Integer.valueOf(param).intValue() > 0
						&& Integer.valueOf(param).intValue() < Integer.MAX_VALUE) {
					timeToJoin = Integer.valueOf(param).intValue();
					msg("Joining period length: "
							+ Integer.toString(timeToJoin) + ".");
				} else if (set.equalsIgnoreCase("gamemaster")
						&& players.contains(getPlayer(param))
					...
					// nextCalled is set to false so that we don't interrupt an
					// ongoing question
					if (param.equals("on")) {
						forceMode = true;
						bonusPaused = false;
						...
						msg("Invalid paramater (expected on or off)");
				} else if (set.equalsIgnoreCase("halftimeLength")
						&& Integer.valueOf(param).intValue() >= 0
						&& Integer.valueOf(param).intValue() < Integer.MAX_VALUE) {
					halftimeLength = Integer.valueOf(param);
					msg("New halftime length: " + halftimeLength + " seconds.");
				} else if (set.equalsIgnoreCase("BreakBtwBonuses")
						&& Integer.valueOf(param).intValue() > 0
						&& Integer.valueOf(param).intValue() < Integer.MAX_VALUE) {
					breakBtwBonuses = Integer.valueOf(param);
					msg("Break between bonus parts now " + breakBtwBonuses
							+ " milliseconds.");
				} else
					msg("Error: Cannot set " + param); // tell them that !set
														// failed
			} catch (NumberFormatException e) {
				msg("Invalid argument.");
				System.out
						.println("Someone tried to use an invalid paramter (integer) in !set");
				e.printStackTrace();
			}
		} else if (isPlaying && text.length() > 8
				&& text.substring(1, 8).equalsIgnoreCase("correct")) {
			int spaceIndex = text.indexOf(" ", 9);
			int points = Integer.valueOf(text.substring(spaceIndex,
					text.length()).trim());
			...
			if (tempPlayer != null && points > 0
					&& points <= Player.getMaxCorrect()) {
				Team tempTeam = getTeam(tempPlayer.getTeam());
				...
				if (tempPlayer == curNegger)
					tempPlayer.unneg();
				msg(tempPlayer.getName() + " was correct. "
						+ Integer.toString(points) + " points given.");
		...
		// bcorrect corrects bonus parts
		else if (isPlaying && text.length() > 9
				&& text.toLowerCase().startsWith("!bcorrect")) {
			if (useBonuses) {
				...
				if (tempTeam.getName().equals(teamName) && points > 0
						&& points <= BonusQuestion.getMaxPoints()) {
		...
		else if (isPlaying && text.length() > 10
				&& text.substring(1, 10).equalsIgnoreCase("incorrect")) {
			int points = -1
					* Integer.valueOf(text.substring(text.indexOf(" ", 11),
							text.length()).trim());
			String playerName = text.substring(11, text.indexOf(" ", 11))
					.trim();
			Player tempPlayer = getPlayer(playerName);
			if (tempPlayer != null && points < 0
					&& points >= -1 * Player.getMaxCorrect()) // getMaxCorrect
																// prevents
																// overflows or
																// other
																// oddities from
																// occurring
			{
				...
				if (tempPlayer == curNegger) // for double !corrects
					tempPlayer.neg();
				msg(tempPlayer.getName() + " was incorrect; "
						+ Integer.toString(-1 * points)
						+ " points were taken away.");
			} else if (tempPlayer == null)
				msg("Player " + playerName + " does not exist");
			else
				msg("Invalid score to use with !incorrect");
		} else if (isPlaying && text.length() > 11
				&& text.substring(1, 11).equalsIgnoreCase("bincorrect")) {
			if (useBonuses) {
				int spaceIndex = text.indexOf(" ", 12);
				int points = -1
						* Integer.valueOf(text.substring(spaceIndex,
								text.length()).trim());
				String teamName = text.substring(11, spaceIndex).trim();
				Team tempTeam = getTeam(teamName);
				if (tempTeam.getName().equals(teamName) && points < 0
						&& points >= -1 * BonusQuestion.getMaxPoints()) {
					tempTeam.addBonusPoints(points);
					tempTeam.addScore(points);
					msg(tempTeam.getName() + " was incorrect; "
		...
		// loads a set
				...
				&& text.substring(1, 8).equalsIgnoreCase("loadset")) {
			qSetFile = text.substring(8, text.length()).trim();
			msg("New set: " + qSetFile);
			loadSet(qSetFile);
			questionNum = questions.size(); // prevents an IndexOutofBounds
											// error
		}

		// Load bonuses
		else if (text.toLowerCase().startsWith("!loadbonuses")
				&& text.length() > 13) {
		...
		// skips the rest of a question
			...
			if (useBonuses && answeringBonus) {
				...
				msg("Possible Answers: " + curquestion.getAnswersString()
						+ " (ID: "
						+ Integer.toString(curquestion.getQuestionNum()) + " "
						+ curquestion.getQuestionSet() + ")");
			}
		} else if (text.toLowerCase().startsWith("!givebonus")
				&& text.length() > 11) {
			if (useBonuses) {
				if (!answeringBonus) {
					...
					for (int i = 0; i < teams.length; i++) {
						if (teams[i].getName().equalsIgnoreCase(giveTeam)) {
							...
							bonusTeam = teams[i];
							startBonus = true;
							roundNum--; // Don't increase the round number...
										// maybe need to decrease by 2 in
										// certain situations
							return;
						}
					}
	...
	private void opCommands(String channel, String sender, String login,
			String host, String text) {
		...
		// loads a player
				...
				&& text.substring(1, 11).equalsIgnoreCase("loadplayer"))
			if (getPlayerFromAllPlayers(text.substring(11, text.length())
					.trim()) == null) {
				Player player = new Player(text.substring(11, text.length())
						.trim());
			...
			String set = arguments[1];
			String param = arguments[2];
			if (set.equalsIgnoreCase("messageDelay")
					&& Integer.valueOf(param).intValue() > 0
					&& Integer.valueOf(param).intValue() < Integer.MAX_VALUE) {
				setMessageDelay(Long.valueOf(param));
				msg("Message delay is now: " + Long.toString(getMessageDelay())
						+ " milliseconds.");
			}
		} else if (text.equalsIgnoreCase("!loadqset")
				|| text.equalsIgnoreCase("!loadqsets")) {
			initSets();
	...
	protected void startJoining(String text) {
		...
		msg("Joining period begins!  " + Integer.toString(timeToJoin)
				+ " seconds!"); // alert the channel members of insuing game
		joining = new Join(1, text); // make a new joining object to accomodate
										// joining
		joining.start();
		gen.setSeed(System.nanoTime() + gen.nextLong()); // reseed the
															// randomizer
	}

	// Adds a player to a team
	protected void addToTeam(String sender, String text) {
		// A player can join a team if it's during the joining period or if the
		// player hasn't been in the game yet
		if (isJoining || isPlaying && getPlayer(sender) == null) {
			...
			// Make sure the team they're joining is valid, // and make sure
			// they're not on another team. --> No longer valid!
			for (int i = 0; i < teams.length; i++) {
				if (temp.equals(teams[i].getName().toLowerCase())
						&& teams[i].size() < playersPerTeam) {
					teams[i].addPlayer(player);
					teams[1 - i].removePlayer(player); // Gets the other team;
														// we've assumed that
														// there are only two
														// teams in this case
					players.add(player);
			...
			if (!temp.equalsIgnoreCase(teams[1].getName())
					&& !temp.equalsIgnoreCase(teams[0].getName()))
	...
	// start(String text): parses team name and start data; uses default if none
	// available//
	// format should be as follows: !start(team1name, team2name,
	// numPeoplePerTeam, numQuestions)
	...
	protected void start(String text) {
		...
		if (text.equalsIgnoreCase("!start"))
			...
			// this uses some magic values: 24 is the current rounds value in
			// settings
			// text += " Home,Away,24,5,.*,(1){0},"+qSetFile; //Adds the most
			// current set file
			text += " " + teams[0].getName() + "," + teams[1].getName() + ","
					+ rounds + "," + playersPerTeam + "," + allowedQ.pattern()
					+ "," + unallowedQ.pattern() + qSetFile;
		...
		// Also, we get all but the last four characters of qSetFile because we
		// do not want the ".txt" portion of it, as that will get added on at
		// the end
		// Note: we set allowedQ to anything and unallowedQ to nothing for the
		// first start
		...
		while (text.indexOf(",", curpos) >= 0 && curpos >= 0
				&& numCommas < params.size()) {
			// If the parameter is not null, then replace the default value with
			// the parameter
			if (!text.substring(curpos, text.indexOf(",", curpos)).trim()
					.equals(""))
				params.set(numCommas,
						text.substring(curpos, text.indexOf(",", curpos))
								.trim());
		...
		if (numCommas < params.size()
				&& !text.substring(curpos, text.length()).trim().equals(""))
		...
		// If the user enters an invalid input, then rounds and playersPerTeam
		// goes back to its previous value
		try {
		...
		if (!allowedQ.pattern().equals(oldAllowedQ)
				|| !unallowedQ.pattern().equals(oldUnallowedQ)) {
			loadSet(qSetFile);
			loadBonuses(bSetFile);
		}
	...
	// it resets the answering variables (answerer, answerTime), and it sets the
	// scores if there are negs
	public void incorrectAnswer(String sender) {
		// NEW ADDITION: kills the timer. Hopefully stops bug where it goes to
		// the next person who buzzed?
		// TODO: Check to see if this is the source of the new
		// NullPointerException. tempPlayer
		// might also be the source.
		cdTimer = null;
		answerer = "";
		answerTime = 0;
		// We make an assumption that sender is a player, since he had to buzz
		// in and answer
		Player tempPlayer = getPlayer(sender);
		// if we're playing with negs, subtract five from the team score
		if (!answeringBonus && negs && !finishedReading && !hasNegged) {
			// if we use !correct, then if that person is curNegger we can fix
			// their neg
	...
	private boolean inPrompts(String text) {
		for (Pattern p : prompts)
			// There won't be many previous prompts, so we can keep rebuilding
			// the regexes
	...
	private void everyoneWrong() // call this when both teams run out of time or
									// get it wrong
	{
		stopQuestion();
		...
		if (answeringBonus) {
			tempQuestion = curbonus.getBonusPart(curBonusPart);
			msg("The correct possible answers: "
					+ tempQuestion.getAnswersString() + " (ID: "
					+ Integer.toString(curbonus.getQuestionNum()) + " "
					+ curbonus.getQuestionSet() + ")");
		} else {
			tempQuestion = curquestion;
			msg("The correct possible answers: "
					+ tempQuestion.getAnswersString() + " (ID: "
					+ Integer.toString(tempQuestion.getQuestionNum()) + " "
					+ tempQuestion.getQuestionSet() + ")");
		}
	}
	...
	protected void onDisconnect() {
		...
		if (attemptReconnect) {
			...
			// If we get some other error (such as an inability to connect to
			// the server),
	...
	// ** Here so that the bot stops playing the game when the only real person
	// quits. Assume that dummy isn't a real person.
	...
	// If someone parts, then we may have no more players left. We check this by
	// calling onQuit
	...
	protected void stillPlayers(String nick) {
		...
		// If the player exists and is the gamemaster, then appoint a new
		// gamemaster.
		if (rem != null && gamemaster.equals(getPlayer(nick))) {
			...
			removePlayer(rem);
			for (Player temp : players) {
				// If there's still a player that's in the room, don't stop the
				// game
	...
	public void askQuestion() {
		if (inOrder == false)
			...
			if (questionNum >= questions.size()) {
				questionNum = gen.nextInt(questions.size()); // Just get a
																// random
																// question
		...
		if (curquestion.getTopic().length() > 13
				&& curquestion.getTopic().substring(0, 13)
						.equalsIgnoreCase("computational"))
			seconds = MATHRND;
		else
			seconds = NORMRND;

		// first line just temporary. Next two lines start the game
		// if showCategory is false, then cut off the category part of the
		// question.
		...
		if (askBySent) {
			...
			if (sentences.length == 0)
				System.out
						.println("Split is the source of the NullPointerException");

			while (curSentence < sentences.length) {
				TBot.maingame.checkPause();
				if (answerTime == 0 && !questionRight) {
					// Display the sentence and it's corresponding punctuation
					// mark
					msg(sentences[curSentence]
							+ punctuation.substring(punctLoc, punctLoc + 1));

					try {
						Thread.sleep(sentences[curSentence].length() * delay);
					} catch (Exception e) {
						System.out
								.println("Exception in askQuestion/askBySent: "
										+ e);
					}
		...
		// Mike Bentley's askQuestion method; modified to work with the current
		// version of the bot
		else {
			String category = "";
			if (showCategory) // if we're showing the category, add a space
			{
				...
				question = question.substring(question.indexOf(".") + 1,
						question.length());
				try {
					Thread.sleep(category.length() * delay);
				} catch (Exception e) {
				}
			}
	...
	private static String getPunctuation(String q) {
		...
		for (int i = 0; i < q.length() - 1; ++i)
			// Make sure there's a space after the punctuation to prevent
			// breaking
			// up abbreviations.
			if (q.substring(i, i + 2).equals(". ")
					|| q.substring(i, i + 2).equals("? ")) {
				retString += q.charAt(i);
				++i;
			}

		// split doesn't take away the last punctuation mark, but an
	...
	public void askBonus() {
		if (curBonusPart == 0) {
			...
			String bonusTeamText = "Bonus for " + bonusTeam.getName()
					+ " Team " + playersString + ":";
			msg(bonusTeamText);
			try {
				Thread.sleep(bonusTeamText.length() * delay);
			} catch (Exception e) {
			}
			...
			questionTime = 0;
			try {
				Thread.sleep(curbonus.getLeadin().length() * delay);
			} catch (Exception e) {
			}
		...
		if (answerTime == 0 && !questionRight) {
			...
			// It's probable that slowness in between bonuses is due to this
			// holding us up after a correct answer
			try {
				Thread.sleep(bonusString.length() * (delay / 2));
			} catch (Exception e) {
			}
	...
	public void MvpLvp() {
		...
		for (Player player : players) {
			// computes the player's score. If it is greater than or matches the
			// mvp's score, (s)he is added to the MVP list.
			// If it is lower than or matches the lvp's score, (s)he is added to
			// the LVP list.
			...
			// If it's the first person being checked, they get put on both
			// lists.
			else {
				lvpList.add(player);
				mvpList.add(player);
		...
		for (Player player : mvpList) {
			player.mvp();
			msg("MVP: " + player.getName() + " on team " + player.getTeam()
					+ " with " + Integer.toString(mvpscore) + " points.");
		}
		for (Player player : lvpList) {
			player.lvp();
			msg("LVP: " + player.getName() + " on team " + player.getTeam()
					+ " with " + Integer.toString(lvpscore) + " points.");
		}
	}
	...
	private static void updateStats(Player p) {
		...
		BufferedWriter filer;
		try {
			filer = new BufferedWriter(new FileWriter(statsDirectory
					+ p.getName() + ".txt", false));
	...
	private class GameThread extends Thread {
		...
		// The main game.
		public void run() {
			// Makes the bot read at least one round (question); fixes a pause
			// bug
			askQuestion();

			// Makes the first question have a countdown: the -1 becomes 0 when
			// it goes through the first iteration
			if (!questionRight)
				questionTime = -1;

			// If there's a game, play the round (question). If there's no game,
			// do nothing.
			while (isPlaying) {
				...
				// This is the whole answer delay bit, when it's after
				// askQuestion()
				//
				// Code that handles if a player has run out of time to answer
				// the question
				// after buzzing in.
				if (answerTime == 1) { // We've run out of time to answer a
										// question after buzzing

					Player tempPlayer = getPlayer(answerer);
					msg(answerer + " [" + tempPlayer.getTeam()
							+ "] ran out of time.");
					answerer = "";
					answerTime = 0;

					// We make an assumption that sender is a player, since he
					// had to buzz in and answer
					// if we're playing with negs, subtract five from the team
					// score
					if (!answeringBonus) {
						...
						// Set "Has answered" to true
						getTeam(tempPlayer.getTeam())
								.setGuessed(true, answerer);
						numGuessed++;
				...
				if (questionTime >= seconds) {
					...
					if (roundNum >= rounds + 1
							...
							|| (questions.size() <= 1 && !answeringBonus)) {
						// If there are no bonuses, check if the game is over
						// since this tossup was read
						if (!useBonuses)
							endOfGame();
		...
		// this pauses and resumes the thread; following example from
		// http://www.devarticles.com/c/a/Java/Multithreading-in-Java/9/
		public void pause() {
			paused = true;
		}
		...
		private void endOfQuestion() {
			...
			if (!questionRight) {
				...
				prompts.clear();
				if (!answeringBonus)
					msg("Time's up.  The correct answer(s): "
							+ curquestion.getAnswersString() + " (ID: "
							+ Integer.toString(curquestion.getQuestionNum())
							+ " " + curquestion.getQuestionSet() + ")");
				else {
			...
			if (!answeringBonus || bonusOver)
				msg(teams[0].getName() + ": "
						+ Integer.toString(teams[0].getScore()) + ", "
						+ teams[1].getName() + ": "
						+ Integer.toString(teams[1].getScore()));
		...
		private void chooseQuestion() {
			...
			if (questionState == TBot.START_TOSSUP) {
				if ((roundNum < rounds + 1 || teams[0].getScore() == teams[1]
						.getScore()) && questions.size() > 1) {
					if (!forceBonus) {
						// Fixed a bug caused by loadSet (loading while
						// questionNum
						// is being chosen).
						if (questionNum < questions.size())
						...
						questionRight = false;
						// give 2 seconds between question # and actual question
						// (2 seconds arbitrary)
				...
				bonusOver = false;
				startBonus = false;
				readFirstBonusPart = true; // read the first bonus part unless
											// interupted by a buzz
				askBonus();
			} else {
		...
		private void endOfGame() {
			msg("You've reached the end of the game!  Score: "
					+ teams[0].getName() + " "
					+ Integer.toString(teams[0].getScore()) + ", "
					+ teams[1].getName() + " "
					+ Integer.toString(teams[1].getScore()));
			// Tell them who the winner is --> this will need to be modified
			// when we increases the # of teams
			...
			if (useBonuses) {
				...
				if (teams[0].getBonusesHeard() == 0)
					...
					ppb = Integer.toString(teams[0].getBonusPoints()
							/ teams[0].getBonusesHeard());
				}
				msg(teams[0].getName() + " Team: Bonuses heard: "
						+ teams[0].getBonusesHeard() + " Bonus points: "
						+ teams[0].getBonusPoints() + " Points per bonus: "
						+ ppb);
				if (teams[1].getBonusesHeard() == 0)
					ppb = "0";
				else
					ppb = Integer.toString(teams[1].getBonusPoints()
							/ teams[1].getBonusesHeard());
				msg(teams[1].getName() + " Team: Bonuses heard: "
						+ teams[1].getBonusesHeard() + " Bonus points: "
						+ teams[1].getBonusPoints() + " Points per bonus: "
						+ ppb);
			}
	...
	private class Join extends Thread {
		...
		public void run() {
			...
			if (answerTime == 0 && !isPlaying) {
				...
				// Check to see if each team has at least one person. If not,
				// end this start
				if (players.size() > 0 && !cancelled) {
					// load the set if gameswoReload != limwoReload, or if the
					// set is lower than the number of rounds
					if (gameswoReload == limwoReload
							|| questions.size() < rounds)// ||
					{
						loadSet(qSetFile);
						loadBonuses(bSetFile);
						gameswoReload = 0;
					...
					questionRight = false;
					msg("Let's Begin!");
					// Gets the first question. If it's a math question
					// (computational or not), make it 30 seconds
					// Change in the future so that it only goes 30 seconds for
					// computational math?
					isPlaying = true;
	...
	private class Countdown extends Thread {
		...
		public void run() {
			...
			if (answerTime == 1) {
				msg(answerer + " [" + (getPlayer(answerer)).getTeam()
						+ "] ran out of time.");
