...
public class Player implements Comparable<Player> {
	...
	// ***************
	/*
	 * name = the player's name. team = the player's team. score = the player's
	 * overall score. stat vars --> numguessed = total number of questions
	 * answered. numcorrect = total number of questions answered correctly.
	 * numgames = total number of games played. mvp = number of games where
	 * (s)he has won the mvp award. lvp = number of games where (s)he has won
	 * the lvp award.
	 */

	private String name, team;
	private int score, numguessed, numcorrect, numgames, numwon, curguessed,
			curcorrect, mvp, lvp, curnegs, numnegs;
	// Used in hashCode().
	private static final int HASH_MULTIPLIER = 37;
	private static final int TUPTS = 10; // Note: TUPTS and NEGPTS may be
											// temporary.
	private static final int NEGPTS = 5;

	// Every variable does not have a get/set function here because it's not
	...
	// Adds the number of points the player got in the round to their overall
	// score
	...
	private void setStats() {
		...
		try {
			...
			while (!eof) {
				...
				if (line == null)
					eof = true;
				else if (line != null && counter < readvars.length) // 4! See
																	// the 4!
																	// That's
																	// important!
				{
	...
	public String getStats() {
		double totalscore = (1.0 * Player.TUPTS) * numcorrect
				- (1.0 * Player.NEGPTS) * numnegs;
		// Split up the string in tbot with ;
		// Make sure the player has played a game; otherwise, we could get a
		...
		if (numgames != 0)
			return name
					...
					+ numgames
					+ ", Percent won: "
					+ Integer.toString((int) (numwon * 1.0 / numgames * 100))
					+ " % , MVPs: "
					+ mvp
					+ ", LVPs: "
					+ lvp
					...
					+ " ("
					+ Integer
							.toString((int) (numcorrect * 1.0 / numguessed * 100))
					+ "% correct), \"PPG\": " + ((int) totalscore / numgames)
					+ ", Total pts: " + ((int) totalscore);
		return name + " has no stats yet, since " + name
				+ " has not completed any games.";
	}

	// Gets the stats for the current game
	public String getCurStats() {
		return name + ": TUs earned: " + curcorrect + ", Negs: " + curnegs
				+ ", TUs answered: " + curguessed + ", Points: "
				+ (Player.TUPTS * curcorrect - Player.NEGPTS * curnegs) + ".";
	}
	...
	public String toString() {
		// return name + " is on team " + team + " and has " +
		// Integer.toString(score) + " points";
		return name;
	}
