...
public class BonusQuestion {
	...
	public BonusQuestion(String line) {
		...
		if (line.charAt(0) == '(') // if it's there
		{
			if (line.indexOf(")") > 0) {
				scoreSpecific = true;
				String[] theScores = line.substring(1, line.indexOf(")"))
						.split(","); // we skip the [ which is at 0
				scores = new int[theScores.length];
				for (int i = 0; i < scores.length; i++)
					scores[i] = Integer.valueOf(theScores[i].trim());
				line = line.substring(line.indexOf(")") + 1, line.length()); // go
																				// one
																				// past
																				// ];
																				// this
		...
		if (theQuestions.length > 1) {
			...
			if (!scoreSpecific) // if scores aren't manually defined, set all
								// scores to NORMAL_BPTS points
			{
	...
	private void updateMaxPoints() {
		// Find the total score possible, see if it's greater than what we have
		// now
		int total = 0;
		for (int i : scores)
			total += i;
